...
def load_comic_dataset():
    dataset = []
    labels = []
    for hero_type in range(CLASS_NUMBER):
        hero_list = listdir("./dataset/{}".format(hero_type))
        for hero_file in hero_list:
            if '.png' or '.jpg' in hero_file:
                path = "./dataset/{}/{}".format(hero_type,hero_file)
                # print(path)
                img = cv2.imread(path,0)
                img = cv2.resize(img, (SIZE, SIZE))
                img = np.reshape(img, [SIZE, SIZE])
                dataset.append(img)
                labels.append(hero_type)
...
def training():
    ...
    evaluate_model(model,data_deskewed,hog_descriptors_test,labels_test)
