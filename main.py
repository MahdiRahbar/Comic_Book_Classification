...
def contourIsHero(perimeter, centroid, threshold):
...
def findLargestHero(image, contours, threshold, distance_theshold):
    max_distance = 0
    coordinate = None
    sign = None
    for c in contours:
        ...
        cX = int(M["m10"] / M["m00"])
        cY = int(M["m01"] / M["m00"])
        is_sign, distance = contourIsHero(c, [cX, cY], 1-threshold)
        if is_sign and distance > max_distance and distance > distance_theshold:
            ...
            sign = cropHero(image,coordinate)
    return sign, coordinate


def findHeros(image, contours, threshold, distance_theshold):
    signs = []
    coordinates = []
    for c in contours:
        ...
        cX = int(M["m10"] / M["m00"])
        cY = int(M["m01"] / M["m00"])
        is_sign, max_distance = contourIsHero(c, [cX, cY], 1-threshold)
...
def localization(image, min_size_components, similitary_contour_with_circle, model, count, current_hero_type):
    ...
    hero, coordinate = findLargestHero(original_image, contours, similitary_contour_with_circle, 15)
    ...
    if hero is not None:
        hero_type = getLabel(model, hero)
        hero_type = hero_type if hero_type <= 8 else 8
        text = HEROS[hero_type]

    if hero_type > 0 and hero_type != current_hero_type:        
    ...
    return coordinate, original_image, hero_type, text
...
def main(args):
    ...
    frame = cv2.imread("dataset/1/5.png")
    ...
    coordinate, image, hero_type, text = localization(frame, args.min_size_components, args.similitary_contour_with_circle, model, count, current_hero)
    if coordinate is not None:
        cv2.rectangle(image, coordinate[0],coordinate[1], (255, 255, 255), 1)
    print("Sign:{}".format(hero_type))
    if hero_type > 0 and (not current_hero or hero_type != current_hero):
        current_hero = hero_type
        current_text = text
        ...
        position = [count, hero_type if hero_type <= 8 else 8, coordinate[0][0], coordinate[0][1], coordinate[1][0], coordinate[1][1]]
        ...
        if hero_type > 0:
            ...
            position = [count, hero_type if hero_type <= 8 else 8, left, top, right, bottom]
            cv2.rectangle(image, coordinate[0],coordinate[1], (0, 255, 0), 1)
            font = cv2.FONT_HERSHEY_PLAIN
            cv2.putText(image,text,(coordinate[0][0], coordinate[0][1] -15), font, 1,(0,0,255),2,cv2.LINE_4)
        elif current_hero:
            position = [count, hero_type if hero_type <= 8 else 8, tl[0], tl[1], br[0], br[1]]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="CV Project Command Line")
    ...
    parser.add_argument(
      ...
      type = float,
      default= 0.45,    # changed from 0.65 to 0.45
